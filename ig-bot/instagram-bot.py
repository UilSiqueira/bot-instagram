...
class IgBot:
    ...
    def __init__(self, username, password):
        ...
        self.driver = webdriver.Firefox(executable_path=r'C:\***put_the _path_here***\geckodriver.exe') 
    ...
    def followers(self):
        ...
        while last_scrl != scrol:
            ...
            seguidores =  self.driver.find_elements_by_class_name("FPmhX")
            seguidores = [elem.get_attribute("title") for elem in seguidores]
            
            self.driver.execute_script("arguments[0].scrollTop = arguments[0].scrollHeight", scrl)
            time.sleep(round(random.uniform(0.75, 1.0), 1))            
            
            last_scrl = scrol  
            scrol = len(seguidores)
